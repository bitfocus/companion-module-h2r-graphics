module.exports = {
	setChoices: function() {
		let self = this;

		self.CHOICES_showhide = [
			{ id: 'show', label: 'Show' },
			{ id: 'hide', label: 'Hide' },
		],
		
		self.CHOICES_pauseresume = [
			{ id: 'pause', label: 'Pause' },
			{ id: 'resume', label: 'Resume' },
		],
		
		self.CHOICES_image = [
			{ id: 'next', label: 'Next' },
			{ id: 'previous', label: 'Previous' },
			{ id: 'hide', label: 'Hide' },
		],
		
		self.GRAPHIC_STATUS_TOGGLES = [
			{ id: 'coming', label: 'Show' },
			{ id: 'going', label: 'Hide' },
			{ id: 'cued', label: 'Cue on' },
			{ id: 'cuedoff', label: 'Cue off' },
		],
		
		self.GRAPHIC_STATUS_OPTIONS = [
			{ id: 'ready', label: 'Ready' },
			{ id: 'cued', label: 'Cue on' },
			{ id: 'coming', label: 'Coming on air' },
			{ id: 'onair', label: 'On air' },
			{ id: 'going', label: 'Going off air' },
			{ id: 'cuedoff', label: 'Cue off' },
			{ id: 'offair', label: 'Off air' },
		],
		
		self.GRAPHIC_POSITION_OPTIONS = [
			{ id: 'tl', label: 'Top Left' },
			{ id: 'tc', label: 'Top Middle' },
			{ id: 'tr', label: 'Top Right' },
			{ id: 'ml', label: 'Middle Left' },
			{ id: 'mc', label: 'Middle' },
			{ id: 'mr', label: 'Middle Right' },
			{ id: 'bl', label: 'Bottom Left' },
			{ id: 'bc', label: 'Bottom Middle' },
			{ id: 'br', label: 'Bottom Right' },
		]

		self.GRAPHICS = [
			{ id: 'lower_third', bgColour: [202, 138, 4], png: ''},
			{ id: 'message', bgColour: [234, 179, 8], png: ''},
			{ id: 'time', bgColour: [248, 113, 113], png: ''},
			{ id: 'image', bgColour: [37, 99, 235], png: ''},
			{ id: 'ticker', bgColour: [165, 180, 252], png: ''},
			{ id: 'social', bgColour: [126, 34, 206], png: ''},
			{ id: 'webpage', bgColour: [55, 48, 163], png: ''},
			{ id: 'score', bgColour: [34, 197, 94], png: ''},
			{ id: 'big_time', bgColour: [248, 113, 113], png: ''},
			{ id: 'lower_third_animated', bgColour: [250, 204, 21], png: ''},
			{ id: 'icon_with_message', bgColour: [234, 179, 8], png: ''},
			{ id: 'credits', bgColour: [34, 197, 94], png: ''},
		]
	}
}